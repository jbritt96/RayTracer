						...
						)
{
	...
	float ECx  = eye.p[0] - s.origin.p[0];
	float ECy  = eye.p[1] - s.origin.p[1];
	float ECz  = eye.p[2] - s.origin.p[2];
	float RAYx = ray.v[0];
	float RAYy = ray.v[1];
	float RAYz = ray.v[2];

	float A = (RAYx * RAYx) + (RAYy * RAYy) + (RAYz * RAYz);
	float B = (2 * ECx * RAYx) + (2 * ECy * RAYy) + (2 * ECz * RAYz);
	float C = (ECx * ECx) + (ECy * ECy) + (ECz * ECz) - (s.radius * s.radius);

	// Check for ray intersecting the sphere with quadratic formula
	float insideSQRT = (B * B) - (4 * A * C);

	
	bool cirlceIntersectedRay = false;
	if (insideSQRT <= 0) {
		return false;
	}

	float Tadd = ((B * -1) + sqrt(insideSQRT)) / (2 * A);
	float Tsub = ((B * -1) - sqrt(insideSQRT)) / (2 * A);

	// Sphere behind eye
	if (Tadd < 0 && Tsub < 0) {
		cirlceIntersectedRay = false;
	}

	// Eye in sphere
	if (Tadd >= 0 && Tsub < 0) {
		distance = Tadd;
		cirlceIntersectedRay = true;
	}
...
void Scene::CastRay(const Vector3 &ray,float &r,float &g,float &b)
{
	...
	{
		//r = g = b = .5;
		// First initialize r,g,b to ambient light
		r = s[closest_hit_sphere].ambient[0];
		g = s[closest_hit_sphere].ambient[1];
		b = s[closest_hit_sphere].ambient[2];

		// Loop through all light sources, adding diffuse and specular lighting
		for (int l = 0; l < this->num_point_lights; l++) {
			...
			if (dot > 0) // we get a diffuse contribution
			{
				...
				if (dot2 > 0) // we have a specular contribution 
				{
